...
interface SidebarItemProps {
  ...
  updateState: any;
  renderInputs: (renderState: any) => any;
}

export const SidebarItem: FC<SidebarItemProps> = ({ item, contentType, updateState, renderInputs, icon, isTyping }) => {
  const { selectedWorkspace, setChats, setSelectedAssistant } = useContext(ChatbotUIContext);
  ...
  return (
    <SidebarUpdateItem item={item} isTyping={isTyping} contentType={contentType} updateState={updateState} renderInputs={renderInputs}>
      <div
        ref={itemRef}
        className={cn('hover:bg-accent flex w-full cursor-pointer items-center rounded p-2 hover:opacity-50 focus:outline-none')}
        tabIndex={0}
      ...
      >
        ...
        <div className="ml-3 flex-1 truncate text-sm font-semibold">{item.name}</div>
