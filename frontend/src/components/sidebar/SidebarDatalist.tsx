...
// export const SidebarDataList: FC<SidebarDataListProps> = ({ contentType, data, folders }) => {
//   const { setChats, setPresets, setPrompts, setFiles, setCollections, setAssistants, setTools, setModels } = useContext(ChatbotUIContext);
...
//   const getDataListComponent = (contentType: ContentType, item: DataItemType) => {
//     switch (contentType) {
//       case 'chats':
...
//   const getSortedData = (data: any, dateCategory: 'Today' | 'Yesterday' | 'Previous Week' | 'Older') => {
//     const now = new Date();
...
//       })
//       .sort(
//         (a: { updated_at: string; created_at: string }, b: { updated_at: string; created_at: string }) =>
...
//     setStateFunction((items: any) => items.map((item: any) => (item.id === updatedItem.id ? updatedItem : item)));
//   };
...
//   useEffect(() => {
//     if (divRef.current) {
//       setIsOverflowing(divRef.current.scrollHeight > divRef.current.clientHeight);
//     }
//   }, [data]);
...
//   return (
//     <>
//       <div ref={divRef} className="mt-2 flex flex-col overflow-auto" onDrop={handleDrop}>
//         {data.length === 0 && (
//           <div className="flex grow flex-col items-center justify-center">
//             <div className=" text-centertext-muted-foreground p-8 text-lg italic">No {contentType}.</div>
//           </div>
//         )}

//         {(dataWithFolders.length > 0 || dataWithoutFolders.length > 0) && (
//           <div className={`h-full ${isOverflowing ? 'w-[calc(100%-8px)]' : 'w-full'} space-y-2 pt-2 ${isOverflowing ? 'mr-2' : ''}`}>
//             {folders.map(folder => (
//               <Folder key={folder.id} folder={folder} onUpdateFolder={updateFolder} contentType={contentType}>
//                 {dataWithFolders
...
//                   const sortedData = getSortedData(dataWithoutFolders, dateCategory as 'Today' | 'Yesterday' | 'Previous Week' | 'Older');

//                   return (
//                     sortedData.length > 0 && (
//                       <div key={dateCategory} className="pb-2">
//                         <div className="text-muted-foreground mb-1 text-sm font-bold">{dateCategory}</div>
