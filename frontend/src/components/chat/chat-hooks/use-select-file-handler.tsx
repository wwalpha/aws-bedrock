...
export const useSelectFileHandler = () => {
  const { selectedWorkspace, profile, chatSettings, setNewMessageImages, setNewMessageFiles, setShowFilesDisplay, setFiles, setUseRetrieval } =
  ...
  const handleFilesToAccept = () => {
    ...
    setFilesToAccept(FULL_MODEL.imageInput ? `${ACCEPTED_FILE_TYPES},image/*` : ACCEPTED_FILE_TYPES);
  };
  ...
  const handleSelectDeviceFile = async (file: File) => {
    ...
    if (file) {
      ...
      if (file.type.includes('image')) {
        ...
        } else if (simplifiedFileType.includes('vnd.openxmlformats-officedocument.wordprocessingml.document')) {
        ...
        if (file.type.includes('vnd.openxmlformats-officedocument.wordprocessingml.document')) {
          ...
          file.type.includes('pdf') ? reader.readAsArrayBuffer(file) : reader.readAsText(file);
        }
      } else {
