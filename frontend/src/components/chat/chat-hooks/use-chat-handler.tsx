...
export const useChatHandler = () => {
  ...
  const handleNewChat = async () => {
    ...
    if (selectedAssistant) {
      ...
      setChatSettings({
        ...
        embeddingsProvider: selectedPreset.embeddings_provider as 'openai' | 'local'
      });
  ...
  const handleSendMessage = async (messageContent: string, chatMessages: ChatMessage[], isRegeneration: boolean) => {
    ...
    try {
      ...
      validateChatSettings(chatSettings, modelData, profile, selectedWorkspace, messageContent);
      ...
      if ((newMessageFiles.length > 0 || chatFiles.length > 0) && useRetrieval) {
        ...
        retrievedFileItems = await handleRetrieval(userInput, newMessageFiles, chatFiles, chatSettings!.embeddingsProvider, sourceCount);
      }
      ...
      let payload: ChatPayload = {
        ...
        chatMessages: isRegeneration ? [...chatMessages] : [...chatMessages, tempUserChatMessage],
      ...
      if (selectedTools.length > 0) {
        ...
        generatedText = await processResponse(
          response,
          isRegeneration ? payload.chatMessages[payload.chatMessages.length - 1] : tempAssistantChatMessage,
          true,
          newAbortController,
          setFirstTokenReceived,
          setChatMessages,
      ...
      if (!currentChat) {
        ...
        setChats(prevChats => {
          const updatedChats = prevChats.map(prevChat => (prevChat.id === updatedChat.id ? updatedChat : prevChat));
  ...
  const handleSendEdit = async (editedContent: string, sequenceNumber: number) => {
    ...
    await deleteMessagesIncludingAndAfter(selectedChat.user_id, selectedChat.id, sequenceNumber);

    const filteredMessages = chatMessages.filter(chatMessage => chatMessage.message.sequence_number < sequenceNumber);
