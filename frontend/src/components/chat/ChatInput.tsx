...
export const ChatInput: FC<ChatInputProps> = ({}) => {
  ...
  const { setNewMessageContentToNextUserMessage, setNewMessageContentToPreviousUserMessage } = useChatHistoryHandler();
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    ...
    if (isPromptPickerOpen || isFilePickerOpen || isToolPickerOpen || isAssistantPickerOpen) {
    ...
    if (isAssistantPickerOpen && (event.key === 'Tab' || event.key === 'ArrowUp' || event.key === 'ArrowDown')) {
      event.preventDefault();
  ...
  const handlePaste = (event: React.ClipboardEvent) => {
    const imagesAllowed = LLM_LIST.find(llm => llm.modelId === chatSettings?.model)?.imageInput;

    const items = event.clipboardData.items;
    for (const item of items) {
      if (item.type.indexOf('image') === 0) {
        if (!imagesAllowed) {
          toast.error(`Images are not supported for this model. Use models like GPT-4 Vision instead.`);
          return;
        }
        ...
        {selectedTools &&
          selectedTools.map((tool, index) => (
            <div
              ...
              onClick={() => setSelectedTools(selectedTools.filter(selectedTool => selectedTool.id !== tool.id))}
            >
