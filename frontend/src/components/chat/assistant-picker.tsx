...
export const AssistantPicker: FC<AssistantPickerProps> = ({}) => {
  const { assistants, assistantImages, focusAssistant, atCommand, isAssistantPickerOpen, setIsAssistantPickerOpen } = useContext(ChatbotUIContext);
  ...
  const filteredAssistants = assistants.filter(assistant => assistant.name.toLowerCase().includes(atCommand.toLowerCase()));
  ...
  const getKeyDownHandler = (index: number) => (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    } else if ((e.key === 'Tab' || e.key === 'ArrowDown') && !e.shiftKey && index === filteredAssistants.length - 1) {
      e.preventDefault();
      itemsRef.current[0]?.focus();
      ...
      {isAssistantPickerOpen && (
        <div className="bg-background flex flex-col space-y-1 rounded-xl border-2 p-2 text-sm">
          {filteredAssistants.length === 0 ? (
            <div className="text-md flex h-14 cursor-pointer items-center justify-center italic hover:opacity-50">No matching assistants.</div>
          ) : (
            <>
              {filteredAssistants.map((item, index) => (
                <div
                  ...
                  onClick={() => callSelectAssistant(item as Tables<'assistants'>)}
                  onKeyDown={getKeyDownHandler(index)}
                >
                  {/* {item.image_path ? (
                    <Image
                      src={assistantImages.find(image => image.path === item.image_path)?.url || ''}
                      alt={item.name}
                      width={32}
                      height={32}
                      className="rounded"
