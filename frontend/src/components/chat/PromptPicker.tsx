...
export const PromptPicker: FC<PromptPickerProps> = ({}) => {
  const { prompts, isPromptPickerOpen, setIsPromptPickerOpen, focusPrompt, slashCommand } = useContext(ChatbotUIContext);
  ...
  const filteredPrompts = prompts.filter(prompt => prompt.name.toLowerCase().includes(slashCommand.toLowerCase()));
  ...
  const getKeyDownHandler = (index: number) => (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.key === 'Backspace') {
      e.preventDefault();
      handleOpenChange(false);
    } else if (e.key === 'Enter') {
    ...
    } else if ((e.key === 'Tab' || e.key === 'ArrowDown') && !e.shiftKey && index === filteredPrompts.length - 1) {
      e.preventDefault();
      itemsRef.current[0]?.focus();
  ...
  const handleSubmitPromptVariables = () => {
    const newPromptContent = promptVariables.reduce(
      (prevContent, variable) => prevContent.replace(new RegExp(`\\{\\{${variable.name}\\}\\}`, 'g'), variable.value),
      prompts.find(prompt => prompt.id === promptVariables[0].promptId)?.content || ''
    );
  ...
  const handleKeydownPromptVariables = (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      {isPromptPickerOpen && (
        <div className="bg-background flex flex-col space-y-1 rounded-xl border-2 p-2 text-sm">
          {showPromptVariables ? (
            <Dialog open={showPromptVariables} onOpenChange={setShowPromptVariables}>
              <DialogContent onKeyDown={handleKeydownPromptVariables}>
                ...
                <div className="mt-2 flex justify-end space-x-2">
                  <Button variant="ghost" size="sm" onClick={handleCancelPromptVariables}>
                    Cancel
                  </Button>
            ...
            <div className="text-md flex h-14 cursor-pointer items-center justify-center italic hover:opacity-50">No matching prompts.</div>
          ) : (
            filteredPrompts.map((prompt, index) => (
              ...
              >
                ...
                <div className="truncate text-sm opacity-80">{prompt.content}</div>
              </div>
            ))
          )}
        </div>
