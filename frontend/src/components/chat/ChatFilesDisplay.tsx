...
import { ChatRetrievalSettings } from './ChatRetrievalSettings';
...
export const ChatFilesDisplay: FC<ChatFilesDisplayProps> = ({}) => {
  ...
  const messageImages = [...newMessageImages.filter(image => !chatImages.some(chatImage => chatImage.messageId === image.messageId))];

  const combinedChatFiles = [...newMessageFiles.filter(file => !chatFiles.some(chatFile => chatFile.id === file.id)), ...chatFiles];
  ...
  return showFilesDisplay && combinedMessageFiles.length > 0 ? (
    <>
      ...
      <div className="space-y-2">
        <div className="flex w-full items-center justify-center">
          <Button className="flex h-[32px] w-[140px] space-x-2" onClick={() => setShowFilesDisplay(false)}>
            <RetrievalToggle />
        ...
        <div className="overflow-auto">
          <div className="flex gap-2 overflow-auto pt-2">
            {messageImages.map((image, index) => (
              <div key={index} className="relative flex h-[64px] cursor-pointer items-center space-x-4 rounded-xl hover:opacity-50">
                {/* <Image
                  className="rounded"
                ...
                <IconX
                  ...
                  onClick={e => {
                    e.stopPropagation();
                    setNewMessageImages(newMessageImages.filter(f => f.messageId !== image.messageId));
                    setChatImages(chatImages.filter(f => f.messageId !== image.messageId));
                  }}
                />
              </div>
            ))}
            ...
            {combinedChatFiles.map((file, index) =>
              file.id === 'loading' ? (
                <div key={index} className="relative flex h-[64px] items-center space-x-4 rounded-xl border-2 px-4 py-3">
                ...
                >
                  <div className="rounded bg-blue-500 p-2">
                    {(() => {
                      let fileExtension = file.type.includes('/') ? file.type.split('/')[1] : file.type;
                  ...
                  <IconX
                    ...
                    onClick={e => {
                      e.stopPropagation();
                      setNewMessageFiles(newMessageFiles.filter(f => f.id !== file.id));
    ...
    combinedMessageFiles.length > 0 && (
      <div className="flex w-full items-center justify-center space-x-2">
        <Button className="flex h-[32px] w-[140px] space-x-2" onClick={() => setShowFilesDisplay(true)}>
          <RetrievalToggle />
  ...
  return (
    <div className="flex items-center">
      {/* <WithTooltip
        ...
        trigger={
          <IconCircleFilled
            className={cn('p-1', useRetrieval ? 'text-green-500' : 'text-red-500', useRetrieval ? 'hover:text-green-200' : 'hover:text-red-200')}
            size={24}
            onClick={e => {
              e.stopPropagation();
