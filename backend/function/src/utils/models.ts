...
const imageGenerationModelId: string = JSON.parse(process.env.IMAGE_GENERATION_MODEL_IDS!)
  .map((name: string) => name.trim())
  .filter((name: string) => name)[0]!;
...
const createBodyImageStableDiffusion = (params: GenerateImageParams) => {
  ...
  if (params.initImage && params.maskImage === undefined) {
    ...
    body = {
      ...
      mask_source: params.maskMode === 'INPAINTING' ? 'MASK_IMAGE_BLACK' : 'MASK_IMAGE_WHITE',
    };
  }
...
const extractOutputImageStableDiffusion = (response: BedrockImageGenerationResponse) => {
...
const extractOutputImageTitanImage = (response: BedrockImageGenerationResponse) => {
