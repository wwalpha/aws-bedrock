...
import { ApiInterface, BedrockImageGenerationResponse, GenerateImageParams, UnrecordedMessage } from 'typings';
...
const initBedrockClient = async () => {
  ...
  if (process.env.CROSS_ACCOUNT_BEDROCK_ROLE_ARN) {
    ...
    const tempCredentials = await assumeRole(process.env.CROSS_ACCOUNT_BEDROCK_ROLE_ARN);
...
const createConverseCommandInput = (model: string, messages: UnrecordedMessage[], id: string): ConverseCommandInput => {
...
const bedrockApi: ApiInterface = {
  invoke: async (model, messages, id): Promise<string> => {
